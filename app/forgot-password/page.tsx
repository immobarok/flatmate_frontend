  ...
  return (
    <main className="flex min-h-dvh w-full items-center justify-center bg-background p-6 selection:bg-primary/30">
      ...
      <Button
        ...
        size="icon"
        onClick={() => router.back()}
        className="absolute left-6 top-6 z-50 rounded-full border-border/50 bg-white/5 backdrop-blur-md dark:bg-stone-900/50"
      >
        ...
        <CardContent className="p-0 text-left">
          {!isSent ? (
            <form onSubmit={onSubmit} className="space-y-5">
              ...
              >
                {isSubmitting ? <Loader2 className="h-6 w-6 animate-spin" /> : "Reset Password"}
              </Button>
            </form>
          ) : (
            <div className="text-center space-y-6">
              ...
              <Button
                ...
                className="h-14 w-full rounded-2xl bg-stone-200 dark:bg-stone-800 text-stone-900 dark:text-white font-bold transition-all hover:bg-stone-300 dark:hover:bg-stone-700"
              >
                Back to Login
              </Button>
            </div>
